...
#define COPYGLYPH_ADJUSTPARTR	0x8		// 16 -> 8ÇÃÇ∆Ç´ÉfÅ[É^Ç™Ç†ÇÈXç¿ïWÇãNì_Ç…ÉRÉsÅ[Åiç∂ëµÇ¶Åj
...
void makepc98bmp(const OEMCHAR *filename, const OEMCHAR* fontface) {
	...
	for (i = 0; i < 0x7f - 0x21; i++) {
		copyglyph(ptr, 0x21 + i, 8, 0x2921 + i, 16, 0); // êîéöâpéöÇ»Ç«
	}
	for (i = 0; i < 0x60 - 0x21; i++) {
		copyglyph(ptr, 0xa1 + i, 8, 0x2a21 + i, 16, 0); // ÉJÉiÇ»Ç«
	}
