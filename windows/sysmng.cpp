...
static bool sortfilefunc(const std::wstring& a, const std::wstring& b) {
	...
	while (*p1 && *p2) {
		...
		if (iswdigit(*p1) && iswdigit(*p2)) {
			...
			// êîílÇ‡í∑Ç≥Ç‡ìØÇ∂ Å® ë±çs
		}
		else {
			// êîílà»äO Å® ïÅí Ç…î‰är
			wchar_t c1 = *p1;
			wchar_t c2 = *p2;
...
static unsigned int __stdcall sysmng_ThreadFuncFindFile(LPVOID vdParam)
{
	...
	do {
		...
		if (args->isCancel) {
			FindClose(hFind);
			goto findfinalize_incs; // ÉLÉÉÉìÉZÉãÇÃèÍçá
		}
...
static void np2updatefddmenu(int drvNo) {
	...
	// ÉhÉâÉCÉué©ëÃÇ™Ç»ÇØÇÍÇŒâΩÇ‡ÇµÇ»Ç¢
	if (!(np2cfg.fddequip & (1 << drvNo))) return;
	
	if (!fdd_diskready(drvNo)) {
		// readyÇ≈Ç»Ç¢Ç»ÇÁç≈å„Ç…äJÇ¢ÇΩFDÉpÉXÇégóp
		szDiskName = fddfolder;

		// ÉtÉ@ÉCÉãÉpÉXÇ™ïœÇÌÇ¡ÇƒÇ¢Ç»Ç©Ç¡ÇΩÇÁÉäÉXÉgçXêVïsóvÇ»ÇÃÇ≈î≤ÇØÇÈ
		if (_tcscmp(np2_fddmenu_base[drvNo], szDiskName) == 0) {
			// ëOâÒë}ì¸èÛë‘ÇæÇ¡ÇΩèÍçáÅAÉ`ÉFÉbÉNÇÕëSÇƒäOÇµÇƒreturn
		...
		// readyÇ»ÇÁÇªÇÃÉtÉ@ÉCÉãÇÃÉpÉXÇégóp
	...
	// Ç‡ÇÃÇ∑Ç≤Ç≠éûä‘Ç™Ç©Ç©Ç¡ÇƒÇµÇ‹Ç§èÍçáÇÃÇΩÇﬂÇ…íZÇﬂÇÃÉ^ÉCÉÄÉAÉEÉgÇê›ÇØÇÈ
	...
	if (WaitForSingleObject(args->completedEvent, 3000) != WAIT_OBJECT_0) {
		...
		if (WaitForSingleObject(args->completedEvent, 0) != WAIT_OBJECT_0) {
			// CSì‡Ç≈äÆóπÇµÇƒÇ¢Ç»Ç¢Ç»ÇÁÉLÉÉÉìÉZÉãÉtÉâÉOÇóßÇƒÇƒÉXÉåÉbÉhÇ…îjä¸ÇîCÇπÇÈ
			CloseHandle(hThreadFind); // ÉXÉåÉbÉhÉnÉìÉhÉãÇÕï¬Ç∂ÇÈÅiÇ‡Ç§ópéñÇ™ñ≥Ç¢ÇÃÇ≈Åj
			args->isCancel = true;
			sysmng_findfile_LeaveCriticalSection();

			// É^ÉCÉÄÉAÉEÉgÇµÇΩÇ∆Ç´Ç‡ÉäÉXÉgéÊìæÇµÇΩÉtÉ@ÉCÉãÉpÉXÇãLâØÇµÇƒÇ®Ç≠ÅiéüâÒÇÕå©Ç…çsÇ©Ç»Ç¢Åj
			_tcscpy(np2_fddmenu_base[drvNo], szDiskName);
			np2_fddmenu_lastready[drvNo] = hasdisk;

			return;
		}
		// åãâ ìIÇ…äÆóπÇµÇΩÇÃÇ≈ë±çs
		sysmng_findfile_LeaveCriticalSection();
	}
	...
	// ÉÅÉjÉÖÅ[Ç…ìoò^ ç≈ëÂÇ≈20ÉtÉ@ÉCÉãÇ‹Ç≈
	i = 0;
	...
	// ÉäÉXÉgéÊìæÇµÇΩÉtÉ@ÉCÉãÉpÉXÇãLâØÇµÇƒÇ®Ç≠ÅiéüâÒè»ó™ÇÃÇΩÇﬂÅj
	_tcscpy(np2_fddmenu_base[drvNo], szDiskName);

	// åªç›ÇÃReadyèÛë‘Çï€ë∂
	np2_fddmenu_lastready[drvNo] = hasdisk;
}
...
void sysmng_requestupdatecaption(UINT8 flag) {
	requestupdate |= flag; // „Éû„É´„ÉÅ„Çπ„É¨„ÉÉ„ÉâÂëº„Å≥Âá∫„ÅóÂØæÁ≠ñÔΩ•ÔΩ•ÔΩ•
}
